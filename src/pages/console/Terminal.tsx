  ...
  const handleCommand = (cmd: string) => {
    ...
    switch (trimmed) {
      ...
      case 'rpt-4502':
        ...
        break
      case '3':
        setOutput(prev => [...prev, '', '═══════════════════════════════════════', '  研究日志', '═══════════════════════════════════════', '', '[2024/03/20] 跨维信息渗透测试 α-7', '目标世界互联网中发现多个与BCMA基础设施', '相对应的通信端点。初步测试已完成。', '', '测试文件已上传至目标世界网盘。', '端点编码: [AES-256 加密]', '测试签名: ECH0-T3ST-077', '', '─── 以下为系统自动记录 ───', '上传位置: Cmh0dHBzOi8vcGFuLnF1YXJrLmNuL3MvOTA0NTFmOTFjYzUzCg==', '状态: 等待对方世界响应', '─── 记录结束 ───', '', '⚠ 警告: 测试文件包含主动频谱信号。', '仅限授权人员访问。', ''])
        break
      case '4':
