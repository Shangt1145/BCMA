  ...
  const handleCommand = (cmd: string) => {
    ...
    switch (trimmed) {
      ...
      case 'rpt-4502':
        ...
        break
      case '3':
        setOutput(prev => [...prev, '', '═══════════════════════════════════════', '  研究日志', '═══════════════════════════════════════', '', '[2024/03/20] 通信异常检测 α-7', '回声分析中心在日常扫描中检测到一个', '未识别的外部通信端点。初步分析显示', '该端点具备数据存储与传输功能。', '端点来源未知，正在溯源分析。', '', '已向该端点部署测试文件以评估其响应。', '测试签名: ECH0-T3ST-077', '', '─── 以下为系统自动记录 ───', '端点位置: aHR0cHM6Ly9wYW4ucXVhcmsuY24vcy9hMGRkMGMyNzI2N2Q=', '状态: 等待响应', '─── 记录结束 ───', '', '⚠ 注意: 测试文件包含主动频谱信号。', '仅限授权人员访问此日志。', ''])
        break
      case '4':
